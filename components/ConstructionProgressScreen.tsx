...
import { ConstructionProgressEntry, Proof, MAX_PROOF_FILE_SIZE_BYTES, ALLOWED_PROOF_FILE_TYPES } from '../types';
...
const ConstructionProgressScreen: React.FC = () => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      const sectionPrefix = "EVOLUCAO_OBRA_";
  ...
  const handleSingleSubmit = (e: React.FormEvent) => {
    ...
    const newEntry: ConstructionProgressEntry = {
      ...
      description: description || undefined, proof: proofData || undefined,
    };
    dispatch({ type: 'ADD_CONSTRUCTION_PROGRESS_ENTRY', payload: newEntry });
    setDate(''); setValue(''); setDescription(''); clearProofFile();
  ...
  const monthlyPayments = useMemo<MonthlyPayment[]>(() => {
    ...
    return Object.entries(grouped)
      ...
      .sort((a, b) => new Date(parseInt(b.monthYear.split('/')[1]), parseInt(b.monthYear.split('/')[0]) -1).getTime() - new Date(parseInt(a.monthYear.split('/')[1]), parseInt(a.monthYear.split('/')[0]) -1).getTime());
  ...
  const handlePrint = async () => {
    await generatePdfFromElement({
      ...
      onFinish: (success) => {
        setIsPrinting(false);
        if (!success) alert("Falha PDF Histórico Evolução Obra.");
        else alert("PDF Histórico gerado. Relatório mensal pode ser gerado separadamente se necessário.");
      }
    });
  };
  ...
  return (
    <div className="space-y-8">
        ...
        <Card title="Novo Pagamento Único de Evolução de Obra">
          <form onSubmit={handleSingleSubmit} className="space-y-4">
            ...
            <div className="mb-4">
                ...
                <input id="cp-proof" type="file" ref={fileInputRef} accept={ALLOWED_PROOF_FILE_TYPES.join(',')}
                ...
                {sortedConstructionProgressEntries.map(entry => (
                  <tr key={entry.id} className="print:text-black">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 print:hidden">
                      {entry.proof ? (
                        <a href={entry.proof.dataUrl} target="_blank" rel="noopener noreferrer" download={entry.proof.name}
      ...
      <Section title="Pagamentos Mensais (Evolução de Obra)" id={CONSTRUCTION_MONTHLY_ID}>
