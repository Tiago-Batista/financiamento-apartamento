...
import { DownPaymentEntry, Proof, MAX_PROOF_FILE_SIZE_BYTES, ALLOWED_PROOF_FILE_TYPES, MAX_PROOF_FILE_SIZE_MB } from '../types';
...
const SECTION_PREFIX = "F_ENTRADA_";
const readableAllowedFileTypes = ALLOWED_PROOF_FILE_TYPES.map(t => t.split('/')[1].toUpperCase()).join(', ');


const DownPaymentScreen: React.FC = () => {
  const { state, dispatch, formatCurrency, getTotalDownPaymentPaid, getDownPaymentRemaining, saveStateToLocalStorageManually, addNotification } = useAppContext();
  ...
  const handleSingleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!date || !value) {
      addNotification('Por favor, preencha a data e o valor.', 'warning');
      return;
    }
    const entryData: DownPaymentEntry = {
    ...
    if (editingEntryId) {
      dispatch({ type: 'UPDATE_DOWN_PAYMENT_ENTRY', payload: entryData });
      addNotification("Entrada atualizada com sucesso!", 'success');
      setEditingEntryId(null); 
    } else {
      dispatch({ type: 'ADD_DOWN_PAYMENT_ENTRY', payload: entryData });
      addNotification("Entrada adicionada com sucesso!", 'success');
    }
    clearSingleForm();
  };

  const handleEditEntry = (entry: DownPaymentEntry) => {
    ...
    setFileError(null);
    const formElement = document.getElementById('dp-single-entry-form-card');
    formElement?.scrollIntoView({ behavior: 'smooth' });
    addNotification(`Editando entrada: ${entry.description || entry.date}`, 'info');
  };

  const handleRemoveEntry = (id: string) => {
    if (window.confirm('Tem certeza que deseja remover esta entrada?')) {
        dispatch({ type: 'REMOVE_DOWN_PAYMENT_ENTRY', payload: id });
        addNotification("Entrada removida.", 'success');
    }
  };
  ...
  const handleBatchSubmit = (e: React.FormEvent) => {
    ...
    addNotification(`${entriesAddedCount} entradas adicionadas em lote!`, 'success');
  };

  const handleSaveData = () => {
    if (saveStateToLocalStorageManually()) {
      addNotification("Dados de F/Entrada salvos localmente!", 'success');
    } else {
      addNotification("Erro ao salvar os dados de F/Entrada.", 'error');
    }
  };

  const handleShareToWhatsApp = () => {
      const totalPaid = getTotalDownPaymentPaid();
      const remaining = getDownPaymentRemaining();
      const message = `Resumo F/Entrada:\nTotal Pago: ${formatCurrency(totalPaid)}\nRestante: ${formatCurrency(remaining)}\n\nÚltimas Entradas:\n${filteredAndSortedDownPaymentEntries.slice(0, 5).map(entry => `- ${new Date(entry.date + 'T00:00:00').toLocaleDateString('pt-BR')}: ${formatCurrency(entry.value)} ${entry.description ? '('+entry.description+')' : ''}`).join('\n')}${filteredAndSortedDownPaymentEntries.length > 5 ? '\n...e mais.' : ''}`;
  ...
  const handleExportCSV = () => {
    if (filteredAndSortedDownPaymentEntries.length === 0) {
      addNotification("Não há dados para exportar.", 'info');
      return;
    }
    const columns: ColumnDefinition<DownPaymentEntry>[] = [
    ...
    const csvData = convertToCSV(filteredAndSortedDownPaymentEntries, columns);
    downloadFile("f_entrada_export.csv", csvData, "text/csv;charset=utf-8;");
    addNotification("Dados exportados para CSV!", 'success');
  };


  const filteredAndSortedDownPaymentEntries = useMemo(() => {
    let entries = [...state.downPaymentEntries];
    if (filterText) {
  ...
  }, [state.downPaymentEntries, filterText, filterStartDate, filterEndDate, filterCategory]);
  ...
  return (
    <div className="space-y-8">
      ...
      />
      <Button onClick={handleExportCSV} variant="primary" size="md" className="print:hidden ml-auto flex items-center" disabled={isPrinting || filteredAndSortedDownPaymentEntries.length === 0}>
      ...
      <Section title="Cadastro em Lote de Entradas (F/Entrada)" className="print:hidden">
        <Card title="Adicionar Múltiplas Entradas">
          <form onSubmit={handleBatchSubmit} className="space-y-4">
            ...
            <div className="mb-4">
              <label htmlFor="dp-batch-category" className="block text-sm font-medium text-gray-700 mb-1">Categoria (Opcional)</label>
              <select id="dp-batch-category" value={batchCategory} onChange={(e) => setBatchCategory(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
                <option value="">Selecione uma categoria</option>
                {state.categories.map(cat => <option key={cat} value={cat}>{cat}</option>)}
              </select>
            </div>
            <div className="mb-4">
              <label htmlFor="dp-batch-proofs" className="block text-sm font-medium text-gray-700 mb-1">
                  Comprovantes da Pasta ({readableAllowedFileTypes} - Máx. {MAX_PROOF_FILE_SIZE_MB}MB por arq.)
              </label>
              <input id="dp-batch-proofs" type="file" ref={batchFileInputRef} {...dirAttributesForBatchUpload} multiple accept={ALLOWED_PROOF_FILE_TYPES.join(',')} onChange={handleBatchFileChange} className="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-primary-light file:text-primary-dark hover:file:bg-primary/80" />
      ...
      <Section title={editingEntryId ? "Editar Entrada (F/Entrada)" : "Cadastro Individual de Entrada (F/Entrada)"} className="print:hidden" id="dp-single-entry-form-card">
        <Card title={editingEntryId ? "Alterar Dados da Entrada" : "Nova Entrada Única"}>
          <form onSubmit={handleSingleSubmit} className="space-y-4">
            <Input label="Data" id="dp-date" type="date" value={date} onChange={(e) => setDate(e.target.value)} required placeholder="DD/MM/AAAA"/>
            <Input label="Valor (R$)" id="dp-value" type="number" value={value} onChange={(e) => setValue(e.target.value)} required min="0" step="any" placeholder="0,00"/>
            <Input label="Descrição (Opcional)" id="dp-description" type="text" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Ex: Pagamento do sinal"/>
            <div className="mb-4">
              <label htmlFor="dp-category" className="block text-sm font-medium text-gray-700 mb-1">Categoria (Opcional)</label>
              <select id="dp-category" value={category} onChange={(e) => setCategory(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
                <option value="">Selecione uma categoria</option>
                {state.categories.map(cat => <option key={cat} value={cat}>{cat}</option>)}
              </select>
            </div>
            ...
            <div className="flex flex-wrap gap-2">
                <Button type="submit" variant="primary" disabled={isPrinting}>{editingEntryId ? "Salvar Alterações" : "Adicionar Entrada (Local)"}</Button>
      ...
      <Section title="Histórico de Entradas" id={DOWNPAYMENT_HISTORY_ID}>
        <Card className="mb-4 p-4 print:hidden">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <Input label="Pesquisar Descrição/Arq." id="filterText" type="text" value={filterText} onChange={e => setFilterText(e.target.value)} placeholder="Ex: Sinal, Boleto_XPTO.pdf" />
            <Input label="Data Inicial" id="filterStartDate" type="date" value={filterStartDate} onChange={e => setFilterStartDate(e.target.value)} />
            <Input label="Data Final" id="filterEndDate" type="date" value={filterEndDate} onChange={e => setFilterEndDate(e.target.value)} />
            <div>
              <label htmlFor="filterCategory" className="block text-sm font-medium text-gray-700 mb-1">Categoria</label>
              <select id="filterCategory" value={filterCategory} onChange={e => setFilterCategory(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
        ...
        {filteredAndSortedDownPaymentEntries.length === 0 ? (
           <EmptyState 
            icon={<DocumentAddIcon />}
            title="Nenhuma Entrada Registrada"
            message={<>Ainda não há entradas de F/Entrada ou os filtros não retornaram resultados. <br/>Clique em "Adicionar Entrada (Local)" acima para começar ou ajuste os filtros.</>}
          />
        ) : (
          <Card className="overflow-x-auto card-print-styling">
            <table className="min-w-full divide-y divide-gray-200 print:divide-gray-300">
              ...
              <tbody className="bg-white divide-y divide-gray-200 print:bg-white print:divide-gray-300">
                {filteredAndSortedDownPaymentEntries.map(entry => (
                  <tr key={entry.id} className="print:text-black">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-sm font-medium print:hidden space-x-2">
                       <Button variant="secondary" size="sm" onClick={() => handleEditEntry(entry)} aria-label="Editar entrada" disabled={isPrinting} className="!p-1.5">
                         <PencilIcon />
                       </Button>
                       <Button variant="danger" size="sm" onClick={() => handleRemoveEntry(entry.id)} aria-label="Remover entrada" disabled={isPrinting} className="!p-1.5">
                         <TrashIcon />
                       </Button>
                    </td>
                  </tr>
